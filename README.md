...
| `features/home/` | `presentation/home_shell_screen.dart` | ログイン後のホーム画面の外枠。下側のタブ切り替え(ホーム/マイページ)を担当 |
| | `presentation/home_tab_screen.dart` | ホームタブの中身(現在は仮の画面) |
| | `presentation/settings_tab_screen.dart` | マイページタブの中身。「プロフィール編集」「ログアウト」ボタンがある |
| `features/profile/` | `presentation/profile_screen.dart` | プロフィール編集画面 |
...
6. features/home/presentation/settings_tab_screen.dart … マイページ画面が表示される
```
