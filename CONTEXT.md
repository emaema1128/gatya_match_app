...
**Gacha**:
The point-costing draw on Home that surfaces one dating candidate at a time, decided in the stage3-gacha-home map (decisions only — not yet implemented, not yet reflected in bloom's backend). Not a bloom-native concept: it's a new spending model layered over the existing `getUserList`/`search` candidate lists via a new `PayCost` category, on a wallet basis (bloom has no stamina/daily-quota precedent). A spin only reveals a candidate (a summary card) — sending a Like is a separate, unpaid action, matching bloom's existing free `sendLike`. Already-Liked/Matched candidates are permanently excluded from the draw; Seen-but-unliked candidates are excluded for a cooldown period, then re-enter the pool. When no eligible candidate remains, a Seen candidate is resurfaced with an on-screen disclosure and a `SERVICE_POINT` bonus rather than an empty state.
_Avoid_: Draw, Roll (use Gacha/Spin, the terms this project settled on), Search (Gacha draws from the same candidate data as the existing `search`/`getUserList` browsing feature, but is a distinct product concept, not a rename of it).
